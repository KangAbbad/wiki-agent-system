...
first=$(printf '%s' "{\"cwd\":\"$workspace\",\"hook_event_name\":\"Stop\",\"session_id\":\"test\",\"turn_id\":\"one\",\"last_assistant_message\":\"Completed migration with token=hidden\"}" | python3 "$plugin_root/hooks/preflight.py")
! printf '%s\n' "$first" | grep -Fq '"decision": "block"'
printf '%s\n' "$first" | grep -Fq '"hookEventName": "Stop"'

second=$(printf '%s' "{\"cwd\":\"$workspace\",\"hook_event_name\":\"Stop\",\"session_id\":\"test\",\"turn_id\":\"one\",\"last_assistant_message\":\"Capture omitted\"}" | python3 "$plugin_root/hooks/preflight.py")
...
printf '%s' "{\"cwd\":\"$workspace\",\"hook_event_name\":\"UserPromptSubmit\",\"session_id\":\"test\",\"turn_id\":\"two\"}" | python3 "$plugin_root/hooks/preflight.py" >/dev/null
python3 "$plugin_root/scripts/wiki_ambient.py" capture --cwd "$workspace" --outcome 'Completed direct capture' --kind result >/dev/null
printf '%s' "{\"cwd\":\"$workspace\",\"hook_event_name\":\"Stop\",\"session_id\":\"test\",\"turn_id\":\"two\",\"last_assistant_message\":\"Capture completed\"}" | python3 "$plugin_root/hooks/preflight.py" | grep -Fq '"hookEventName": "Stop"'
